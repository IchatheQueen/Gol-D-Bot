...
const command: Command = {
    ...
    execute: async (message: Message, args: string[], client: Client) => {
        ...
        weaponsContent += `[ID: 12] ${resolveEmoji(client, flamethrower.emoji)} **${flamethrower.name}** - Price: 🍺 ${formatBigNumber(flamethrower.price)}\n   └ ${flamethrower.description}\n`;
        ...
        ammoContent += `[ID: 8] ${resolveEmoji(client, arrow.emoji)} **${arrow.name}** - Price: 🍺 ${formatBigNumber(arrow.price)}\n   └ ${arrow.description}\n`;
        ammoContent += `[ID: 10] ${resolveEmoji(client, rifleBullet.emoji)} **${rifleBullet.name}** - Price: 🍺 ${formatBigNumber(rifleBullet.price)}\n   └ ${rifleBullet.description}\n`;
        ammoContent += `[ID: 13] ${resolveEmoji(client, propane.emoji || '🛢️')} **${propane.name}** - Price: 🍺 ${formatBigNumber(propane.price)}\n   └ ${propane.description}\n`;
